...
router.get('/book/edit/:id(\\d+)', csrfProtection, asyncHandler(async (req, res) => {
	...
	res.render('book-edit', { title: 'Edit Book', book, csrfToken: req.csrfToken() });
}));
...
router.post('/book/edit/:id(\\d+)', csrfProtection, bookValidators, asyncHandler(async (req, res) => {
	...
	const book = { title, author, releaseDate, pageCount, publisher };
	...
	if (validatorErrors.isEmpty()) {
		...
		const errors = validatorErrors.array().map(error => error.msg);
		res.render('book-edit', { title: 'Edit Book', book: { ...book, id: bookId }, errors, csrfToken: req.csrfToken() });
	}
}))
...
router.get('/book/delete/:id(\\d+)', csrfProtection, asyncHandler(async (req, res) => {
	const bookId = parseInt(req.params.id, 10);
	const book = await db.Book.findByPk(bookId);
	res.render('book-delete', { title: 'Delete Book', book, csrfToken: req.csrfToken() });
}));
